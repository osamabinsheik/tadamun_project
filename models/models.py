...
class Activities(models.Model):
	...
	jan = fields.Boolean('Jan')
	feb = fields.Boolean('Feb')
	mar = fields.Boolean('Mar')
	apr = fields.Boolean('Apr')
	may = fields.Boolean('May')
	jun = fields.Boolean('Jun')
	jul = fields.Boolean('Jul')
	aug = fields.Boolean('Aug')
	sep = fields.Boolean('Sep')
	oct = fields.Boolean('Oct')
	nov = fields.Boolean('Nov')
	dec = fields.Boolean('Dec')
...
class Staff(models.Model):
	...
	budget_line_description = fields.Char(string = "Budget Line Description")
	remarks = fields.Text('Remarks')
	d_and_s = fields.Selection([
        ('d', 'D'),
        ('s', 'S'),
        ], string='D/S Direct,Support', default='d')
	unit_quantity = fields.Integer('Unit Quantity')
	unit_cost = fields.Integer('Unit Cost')
	duration = fields.Integer('Duration (per month, per day or lump sum)')
	total_cost = fields.Float('Total Cost % charged to donor')
	total = fields.Float('Total', compute='_compute_budgets', readonly=True, store=True)
	budget_desc = fields.Text()
	project_id = fields.Many2one('project.project')
